...
class TaskConfigBase(BaseModel):
    ...
    max_instances: int = Field(1, description="最大实例数", ge=1, le=100)
...
class TaskConfigCreate(TaskConfigBase):
    ...
    def validate_parameters(cls, v, values):
        ...
        # 清理任务参数验证  
        elif task_type in [TaskType.CLEANUP_SESSIONS, TaskType.CLEANUP_CONTENT, TaskType.CLEANUP_EVENTS]:
...
class TaskConfigUpdate(BaseModel):
    ...
    max_instances: Optional[int] = Field(None, description="最大实例数", ge=1, le=100)
